...
class NoteServiceTest {
    ...
    @Test
    fun add() {
        val actual = NoteService.add(Note(noteId = 0, title = "title", text = "text"))

        assertFalse("Ожидалось, что добавленная заметка будет иметь id отличный от нуля", actual == 0)
    }

    @Test
    fun delete() {
        val noteId = NoteService.add(Note(noteId = 0, title = "title", text = "text"))
    ...
    fun edit() {
        val noteId = NoteService.add(Note(noteId = 0, title = "title", text = "text"))
        val newNote = Note(id = noteId, noteId = 0, title = "new_title", text = "new_text")
    ...
    @Test
    fun read() {
        NoteService.add(Note(noteId = 0, title = "title", text = "text"))
    ...
    @Test
    fun getById() {
        val expected = NoteService.add(Note(noteId = 0, title = "title", text = "text"))
    ...
    fun createComment() {
        ...
        val expected = NoteService.createComment(noteId = noteId, ownerId = 0, replyTo = 0, commentId = 0, message = "message")
    ...
    @Test
    fun deleteComment() {
        val commentId = NoteService.createComment(noteId = 0, ownerId = 0, replyTo = 0, commentId = 0, message = "message")
    ...
    fun editComment() {
        val commentId = NoteService.createComment(noteId = 0, ownerId = 0, replyTo = 0, commentId = 0, message = "message")
        NoteService.editComment(id = commentId, ownerId = 0, message = "new_message")
    ...
    fun getComments() {
        val noteId = NoteService.add(Note(noteId = 0, title = "title", text = "text"))
        NoteService.createComment(noteId = noteId, ownerId = 0, replyTo = 0, commentId = 0, message = "message")
